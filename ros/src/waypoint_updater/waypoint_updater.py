...
class WaypointUpdater(object):
    ...
    def get_closest_waypoint_id(self):
        ...
        pre_closest_coord = self.waypoints_2d[closest_idx - 1]

        closest_vec = np.array(closest_coord)
        pre_closest_vec = np.array(pre_closest_coord)
        pose_vec = np.array([x, y])

        val = np.dot(closest_vec - pre_closest_vec, closest_vec - pose_vec)
        if val > 0:
